...
```bash
podman build -t nyancat-strace:latest -f strace/Dockerfile .
```
...
```bash
cd strace
kind create cluster --name nyancat-strace --config kind-config.yaml
```
...
podman save --format oci-archive -o nyancat-strace.tar nyancat-strace:latest

# Load the image archive into kind
kind load image-archive nyancat-strace.tar --name nyancat-strace
```
